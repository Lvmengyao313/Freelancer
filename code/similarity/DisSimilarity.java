...
//		return 1 - (double) ld / Math.max(src.length(), tar.length()); 
		return 1/(1+ (double) ld / Math.max(src.length(), tar.length())); 
	}
	
	
	public static void main(String[] args) {
		String src = "website redesign";
		String tar = "website re-design";
		System.out.println("sim="+DisSimilarity.similarity(src, tar));
	}
}